...
function create(session, catchQR, statusFind, options) {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var spinnies, mergedOptions, waPage, authenticated, _a, data, asciiQR, localStorage_1, _b, _c, WABrowserId_1, WASecretBundle_1, WAToken1_1, WAToken2_1, debugURL;
        return __generator(this, function (_d) {
            switch (_d.label) {
                ...
                case 4:
                    ...
                    if (!(mergedOptions.refreshQR <= 0 || mergedOptions.autoClose <= 0)) return [3 /*break*/, 6];
                ...
                case 11:
                    ...
                    try {
                        setTimeout(function () {
                            fs_1.mkdir(path.join(path.resolve(process.cwd(), 'tokens')), { recursive: true }, function (err) {
                                if (err) {
                        ...
                        setTimeout(function () {
                            fs_1.writeFileSync(path.join(path.resolve(process.cwd(), 'tokens'), session + ".data.json"), JSON.stringify({ WABrowserId: WABrowserId_1, WASecretBundle: WASecretBundle_1, WAToken1: WAToken1_1, WAToken2: WAToken2_1 }));
 ...
 */
var countDown = function (msTimeOut) { return counter.getElapsedTime() < msTimeOut; };
/**
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        ...
                        if (!countDown(options.autoClose)) return [3 /*break*/, 1];
                        _b = null;
                        return [3 /*break*/, 3];
                    case 1: return [4 /*yield*/, waPage.close()];
                    case 2:
                        _b = _c.sent();
                        _c.label = 3;
                    case 3:
                        _b; //Close Imediatly
                        timeOut = Math.round((options.autoClose - counter.getElapsedTime()) / 1000);
                        if (catchQR) {
                            catchQR(data, asciiQR);
                        }
                        if (options.logQR) {
                            console.clear();
                            console.log('Scan QR for: ' +
                                session +
                                '                ' +
...
function logUpdateAvailable(current, latest) {
    ...
    console.log("For more info visit: " + chalk.underline('https://github.com/orkestral/venom/blob/master/UPDATES.md') + "\n");
}
